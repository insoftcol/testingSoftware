...
- **Capa afectada**: Aplicación (`ServicioTarifa`)
- **Caso de prueba**: `procesarTransferencia(null)` / `cotizar(null)`
- **Entrada**: `null`
- **Resultado esperado**: resultado DATOS_INVALIDOS guardado en `cotizacion_log`
- **Resultado obtenido**: `NullPointerException` — la tabla no recibía el registro
- **Causa probable**: El método `save()` en `CotizacionRepository` no manejaba valores nulos en `solicitud`
- **Tipo de prueba**: Integración (H2)
- **Estado**: Resuelto — se agregaron comprobaciones de nulo en `CotizacionRepository.save()`
- **Prioridad**: Alta
...
- **Capa afectada**: Delivery (`CotizacionController`)
